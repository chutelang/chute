...
The shortcut defines a `cleanURL` function that splits a URL on the `?` character and returns the part before it — the base URL without any query string. In Chute, functions compile to separate sub-shortcuts that the main shortcut calls with "Run Shortcut."
